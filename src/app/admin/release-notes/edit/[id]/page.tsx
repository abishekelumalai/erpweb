  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const data = await res.json();
        toast.error(data.error || 'Failed to update release note');
      }
    } catch {
  ...
  return (
    <div className="space-y-6 max-w-2xl">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-2xl font-bold tracking-tight">Edit Release Note</h1>
          <p className="text-muted-foreground">Update the release note details.</p>
        </div>
      </div>
